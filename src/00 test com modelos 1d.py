...
SAMPLE_RATE = 16000
DURATION = 1.5
TOTAL_SAMPLES = int(SAMPLE_RATE * DURATION)  # 24000 pontos

# Adicionada a classe "ruido" na lista oficial (Agora são 6 classes: 0 a 5)
...
def trim_manual(audio, top_db=25, frame_length=2048, hop_length=512):
    """Remove o silêncio inicial e final calculando a energia em decibéis"""
    if len(audio) == 0:
        return audio
        ...
        for arquivo in arquivos:
            if arquivo.lower().endswith(".wav"):
                ...
                try:
                    ...
                    # ATENÇÃO ESPECIAL: Se for a classe de ruído de fundo contínuo, 
                    # talvez o trim remova o áudio inteiro se o volume for baixo estável.
                    # Por segurança, aplicamos o trim apenas nas palavras.
...
# Gráfico 2: Matriz de Confusão (Agora 6x6)
...
plt.title('Matriz de Confusão - Com Classe Ruído')
plt.xlabel('Predito')
plt.ylabel('Real')
plt.tight_layout()
...
# Gráfico 3: Curva ROC Multiclasse (One-vs-Rest)
...
plt.grid(True)
plt.tight_layout()
plt.savefig("hibrido_curva_roc.png", dpi=300)
plt.show()

print("\n[PROCESSO CONCLUÍDO] Dataset 'dataset_final' treinado com a classe de ruído inclusa!")