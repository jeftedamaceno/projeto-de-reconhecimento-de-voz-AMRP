...
# with open("labels_1s5_atencao_v2.json", "r") as f:
...
# import scipy.io.wavfile as wav  # Certifique-se de adicionar este import no topo do arquivo

# def audio_stream_thread():
#     BLOCO_SAMPLES = int(SAMPLE_RATE * 0.5) 
#     audio_queue = queue.Queue()
#     historico_blocos = []
#     cooldown_tempo = 0
    
#     # Controle para salvar áudio a cada 1 segundo
#     ultima_gravacao_tempo = 0
#     contador_audio = 0
#     pasta_audios = "audios_ia"
...
#     with sd.InputStream(samplerate=SAMPLE_RATE, channels=1, blocksize=BLOCO_SAMPLES, dtype='float32', callback=callback):
#         while True:
#             bloco_atual = audio_queue.get()
#             historico_blocos.append(bloco_atual)
            
#             if len(historico_blocos) > 3:
#                 historico_blocos.pop(0)
...
#             if len(historico_blocos) == 3:
#                 audio_buffer = np.concatenate(historico_blocos)
                
#                 audio_int16 = (audio_buffer * 32767.0).astype(np.float32)
#                 rms = np.sqrt(np.mean(audio_int16**2))
                
#                 if rms > 150.0:  
#                     if np.std(audio_int16) > 1e-6:
#                         audio_preprocessed = (audio_int16 - np.mean(audio_int16)) / np.std(audio_int16)
#                     else:
#                         audio_preprocessed = audio_int16
                    
#                     # --- NOVO: SALVAR O ÁUDIO ENVIADO PARA A IA ---
#                     tempo_atual = time.time()
#                     if tempo_atual - ultima_gravacao_tempo >= 1.0: # Garante intervalo mínimo de 1 segundo
#                         # Convertemos de volta para int16 normalizado para gerar um arquivo WAV audível
#                         audio_para_salvar = np.clip(audio_int16, -32768, 32767).astype(np.short)
#                         caminho_arquivo = os.path.join(pasta_audios, f"audio_{contador_audio}.wav")
#                         wav.write(caminho_arquivo, SAMPLE_RATE, audio_para_salvar)
                        
#                         print(f"[DEBUG] Áudio salvo em: {caminho_arquivo} (RMS: {rms:.1f})")
#                         contador_audio += 1
#                         ultima_gravacao_tempo = tempo_atual
#                     # -----------------------------------------------
...
#     pygame.display.set_caption("Robo - Escuta Ativa Continua Sem Botao V2")
...
#         lbl_historico = font_bold.render("ESCUTA ATIVA CONTINUA (JANELA DESLIZANTE):", True, (240, 240, 240))
#         screen.blit(lbl_historico, (20, hud_top_y + 15))
        
#         texto_linha_historico = " -> ".join(historico_palavras_ditas) if historico_palavras_ditas else "Fale comandos a qualquer momento..."
...
#             bar_color = (0, 255, 150) if prob > 0.70 else (255, 165, 0) if prob > 0.30 else (100, 100, 110)
